...
async function run() {
  try {
    ...
    // Add Pollinations AI for generative perfect images!
    const P = (prompt) => `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=800&height=800&nologo=true`;
    ...
    function getFallback(key, name) {
      ...
      if (key.includes("suoi")) return FALLBACKS.water;
      
      // Use Generative AI for drinks to get perfect pictures of bubble tea, soda, peach tea etc.!
      return P(name + " refreshing drink glass, high quality food photography, vibrant"); 
    }
    ...
    const imagePool = {
      ...
      // Removed broken static IDs for drinks so they will fallback to Pollinations AI generative!
      "nuoc-suoi":            U("1548839140-29a749e1cf4d"),
      "ca-phe-den":           U("1509042239860-f550ce710b93"),
      // ca-phe-sua (broken), tra-sua-tran-chau, vv.. sẽ tự động rớt xuống fallback.
    };
    ...
    function makeProductInfo({
      ...
      const img = imagePool[key] ?? getFallback(key, name);
      return {
        _class: className,
        name,
        description,
    ...
    for (const p of allProductInfos) {
      ...
      // We skip validation for generative AI URLs because they are dynamically generated and always work
      if (finalUrl.includes('pollinations.ai')) {
         // Do nothing, keep finalUrl as is
      } else if (!urlCache.has(finalUrl)) {
        try {
          const res = await fetch(finalUrl, { method: 'HEAD' });
          if (!res.ok) {
            console.log("Broken:", finalUrl, "Fallback to intelligent category match...");
            finalUrl = getFallback(p.key, p.name);
          }
        } catch (e) {
