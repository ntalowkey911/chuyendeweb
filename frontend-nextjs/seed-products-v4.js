...
async function run() {
  try {
    ...
    const PICSUM = (key) => `https://picsum.photos/seed/${key}/800/800`;
    ...
    function makeProductInfo({
      ...
      return {
        ...
        description,
        price,
        imageUrl: img, // Will be validated and replaced if broken
        imageUrls: [img],
        ...
        key // Keep key for picsum fallback
      };
    }
    ...
    for (const p of allProductInfos) {
      ...
      if (!urlCache.has(finalUrl)) {
        try {
          ...
          if (!res.ok) {
            console.log("Broken:", finalUrl, "Fallback to picsum...");
            finalUrl = PICSUM(p.key);
          }
        } catch (e) {
           console.log("Error checking:", finalUrl, "Fallback to picsum...");
           finalUrl = PICSUM(p.key);
        }
